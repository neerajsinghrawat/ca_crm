...
class iv_intrested_vehicleController extends SugarController
{
    ...
    public function action_used_sold()
    {
        $_REQUEST['form_of_payment'] = 'used';
        $this->view = 'lease_sold';
    }
    ...
    public function action_getSoldVehicles()
    {
        ...
        LEFT JOIN ac_dealers ac 
            ON ac.id = iv.account_id_c
            AND ac.deleted = 0
        ...
        <th class="pd-add"><div>Vehicle</div></th>
        <th class="pd-add"><div>Lead Name</div></th>
        <th class="pd-add"><div>Dealer</div></th>
        <th class="pd-add"><div>Status</div></th>
        <th class="pd-add"><div>Payment Type</div></th>

        <th class="pd-add"><div>VIN</div></th>
        <th class="pd-add"><div>Year</div></th>
        <th class="pd-add"><div>Make</div></th>';
        if($type == 'lease'){
            $html .= '
            <th class="pd-add"><div>Start Date</div></th>
            <th class="pd-add"><div>End Date</div></th>
            <th class="pd-add"><div>Term</div></th>
            <th class="pd-add"><div>Miles / Year</div></th>
            <th class="pd-add"><div>Monthly Payment</div></th>
            <th class="pd-add"><div>Bank</div></th>
            <th class="pd-add"><div>Month</div></th>';
        }
        $html .= '<th class="pd-add"><div>Date Created</div></th>
        </tr>
        </thead><tbody>';
        ...
        while($row = $db->fetchByAssoc($result)){
            ...
            $html .= "<td><a target='_blank' href='index.php?module=ac_dealers&action=DetailView&record=".$row['dealer_id']."'>".$row['dealer_name']."</a></td>";
    ...
    public function action_updateVehicleFields()
    {
        ...
        foreach ($ids as $id) {
            ...
            if (!empty($_POST['dealer_id'])) {
                $fields[] = "account_id_c='".$db->quote($_POST['dealer_id'])."'";
            }
            if (!empty($_POST['form_of_payment'])) {
                $fields[] = "form_of_payment='".$db->quote($_POST['form_of_payment'])."'";
            }
